...
Every invocation of `install-statusline.sh` — Step 3 and the `--help` route alike —
goes through the block below. It is the only place this skill spells the path.

```sh
[ -n "${CLAUDE_PLUGIN_ROOT:-}" ] || {                                                # tier 5
    printf '[claudecode:statusline-setup] CLAUDE_PLUGIN_ROOT is unset, so the bundled script cannot be located. On Claude Code this is a broken install; on any other harness export CLAUDE_PLUGIN_ROOT=<the directory you read SKILL.md from> first.\n' >&2
    return 1 2>/dev/null || exit 1
}
_SL="$CLAUDE_PLUGIN_ROOT/skills/statusline-setup/scripts/install-statusline.sh"       # tier 2
[ -r "$_SL" ] || {                                                                   # tier 5
    printf '[claudecode:statusline-setup] %s is not readable — broken install.\n' "$_SL" >&2
    return 1 2>/dev/null || exit 1
}
```
...
```sh
sh "$_SL" --dry-run                                    # print, write nothing
sh "$_SL" [--usage-id ID --usage-api URL] [--budget N] # the install
sh "$_SL" --help                                       # the help route
```
