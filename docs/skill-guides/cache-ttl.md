...
2. **스크립트 실행** — `references/run-script.md` 의 locator 블록으로 `$_CC` 를 잡은 뒤
   `sh "$_CC" <mode>`. 모델이 `jq` 를 직접 타이핑하지 않는 것이 핵심입니다.
   플러그인 루트는 `$CLAUDE_PLUGIN_ROOT` 가 유일한 출처이며, 미설정이면 cwd 로 떨어지지 않고
   멈춥니다 (retired tier 4, `harness-skills#22`).
