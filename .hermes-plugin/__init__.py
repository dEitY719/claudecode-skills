"""Hermes Agent registration for the `claudecode` skills plugin.

Registers the two Claude-Code-configuration skills with Hermes' native skill
loader so `skill_view("claudecode:<name>")` can load them on demand.

Like the sibling `authoring` plugin, this one injects no session bootstrap
context: both skills are task-triggered (you reach for one when the prompt
cache TTL or the status line needs changing), so there is nothing worth paying
for on every first turn.
"""
...
_SENTINEL = ("cache-ttl", "SKILL.md")


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install dEitY719/claudecode-skills`):
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, *_SENTINEL)):
            return cand
    raise RuntimeError(
        "claudecode plugin: cannot find the skills/ tree "
